...
class _InsertMenuState extends State<InsertMenu> {
  ...
  Future<void> _insertDrawing() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Drawing canvas coming soon')),
    );
  }
  ...
  Future<void> _insertDateTime() async {
    ...
    if (format != null) {
      ...
      switch (format) {
        case 'DD/MM/YYYY':
          dateTimeStr = '${now.day.toString().padLeft(2, '0')}/${now.month.toString().padLeft(2, '0')}/${now.year}';
          break;
        case 'MM/DD/YYYY':
          dateTimeStr = '${now.month.toString().padLeft(2, '0')}/${now.day.toString().padLeft(2, '0')}/${now.year}';
          break;
        case 'full':
