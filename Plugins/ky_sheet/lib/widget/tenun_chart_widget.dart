...
class _TenunChartWidgetState extends State<TenunChartWidget> {
      ...
      data: ChartData(
        ...
        datasets: seriesList
            .map((s) => ChartDataset(
                  label: s.name,
                  data: s.values,
                  backgroundColor: _getChartColors().take(s.values.length).toList(),
                ))
            .toList(),
      ),
  ...
  Widget _buildLineChart(List<String> categories, List<ChartSeries> seriesList) {
    return LineChart(
      data: ChartData(
        labels: categories,
        datasets: seriesList
            .map((s) => ChartDataset(
                  ...
                  backgroundColor: _getChartColors()[seriesList.indexOf(s)].withOpacity(0.1),
                  fill: false,
                ))
            .toList(),
      ),
      ...
      data: ChartData(
        ...
        datasets: [
          ChartDataset(
            ...
            backgroundColor: _getChartColors().take(firstSeries.values.length).toList(),
          ),
        ],
      ),
  ...
  Widget _buildLegend() {
    final labels = widget.type == SheetChartType.pie
        ? widget.data.series.firstOrNull?.points.map((p) => p.label).take(6).toList() ?? []
