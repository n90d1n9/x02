...
class PrintService {
  ...
  Future<PrintResult> preview(PrintJob job, {BuildContext? context}) async {
    try {
      ...
      if (context != null) {
        ...
        // Return preview data
        return PrintResult.success(pdfPath: 'preview', pagesPrinted: job.totalPages);
      }
    } catch (e) {
  ...
  Future<PrintResult> saveToPdf(PrintJob job, {String? fileName}) async {
    try {
      ...
      final safeFileName = fileName ?? '${job.documentTitle.replaceAll(RegExp(r'[^\w\s-]'), '_')}.pdf';
      final filePath = path.join(directory.path, safeFileName);
      
      final file = await File(filePath).writeAsBytes(pdfBytes);
      
      return PrintResult.success(pdfPath: file.path, pagesPrinted: job.totalPages);
    } catch (e) {
