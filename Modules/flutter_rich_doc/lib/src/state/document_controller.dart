...
import 'flutter_rich_doc/lib/src/rust/api/document.dart' as rust;
...
class DocumentController extends Notifier<DocumentState> {
  ...
  bool _applyAndTrack(String commandJson) {
    final resultJson = rust.applyCommand(
      handle: _handle,
  ...
  DocumentState _fetchState() {
    final json =
        jsonDecode(rust.getSnapshot(handle: _handle)) as Map<String, dynamic>;
    return DocumentState(
      tree: DocumentTree.fromJson(json),
      canUndo: canUndo,
      canRedo: canRedo,
    );
  }
...
final documentControllerProvider =
    NotifierProvider<DocumentController, DocumentState>(DocumentController.new);
