...
use serde::Serialize;
use pptx_reader::{Geometry, Presentation, Shape, Slide, SlideRenderer, Fill, Stroke, TextBox, TextRun, TextAlign, VerticalAlign};
                ...
                if let Some(embed_end) = embed_rest.find('\"') {
                    ...
                    if let Some(image_data) = load_image_by_rel_id(rel_id, archive) {
                        ...
                        shape.fill = Some(Fill::Picture {
                            ...
                            fit: pptx_reader::ImageFit::Contain,
                        });
...
#[no_mangle]
pub extern "C" fn pptx_reader_version() -> *mut c_char {
    string_to_raw("pptx_reader v0.1.0")
}

#[no_mangle]
pub extern "C" fn pptx_reader_free_string(s: *mut c_char) {
    if s.is_null() {
        return;
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn pptx_reader_free_presentation(pres: *mut Presentation) {
    if pres.is_null() {
        return;
    }
    unsafe {
...
// ---------------------------------------------------------------------
// Safety: callers must free returned strings with `pptx_reader_free_string`
// and presentations with `pptx_reader_free_presentation`.
// ---------------------------------------------------------------------

#[cfg(test)]
mod tests {
    use super::*;
    use pptx_reader::{DrawCommand, Rect};
    use std::ffi::{CStr, CString};

    fn take_string(ptr: *mut c_char) -> String {
        assert!(!ptr.is_null());
        let value = unsafe { CStr::from_ptr(ptr) }.to_str().unwrap().to_owned();
        pptx_reader_free_string(ptr);
        value
    }
    ...
    fn serialize_and_deserialize_presentation() {
        ...
        pptx_reader_free_presentation(ptr);
    }
    ...
    fn add_and_move_shape_returns_render_commands() {
        ...
        pptx_reader_free_presentation(pres_ptr);
    }
}
