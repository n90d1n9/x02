...
```toml
[dependencies]
parser-xlsx = { path = "." }              # local
# parser-xlsx = "0.1"                     # crates.io (once published)
```

```rust
use parser-xlsx::{Workbook, WorkbookReader};

fn main() -> parser-xlsx::Result<()> {
...
```rust
use parser-xlsx::iter::StreamingReader;
