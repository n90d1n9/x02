...
pub fn grid_to_write_request(
    ...
    grids: &[SheetGrid],
) -> Result<parser-xlsx::writer::XlsxWriteRequest, XlsxWorkbookError> {
...
pub fn write_grids_to_workbook(
    ...
    parser-xlsx::writer::write_xlsx(&write_request)
...
fn append_grid_cells(
    request: &mut parser-xlsx::writer::XlsxWriteRequest,
    sheet_name: &str,
    grid: &SheetGrid,
...
fn cell_position_to_a1(
    ...
    Ok(parser-xlsx::CellAddress::new(position.row, col).to_a1())
}
