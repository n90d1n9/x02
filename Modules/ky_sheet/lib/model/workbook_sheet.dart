...
class SheetWorkbook {
  ...
  int get totalCellCount => sheets.fold<int>(0, (sum, sheet) => sum + sheet.cells.length);
  
  String? get name => null; // Workbook name - can be set separately
  
  String? get currentFilePath => null; // Current file path - can be set separately
  
  void setCurrentFilePath(String filePath) {
    // TODO: Store current file path
  }
}
...
class WorkbookSheet {
  ...
  int get rowCount => cells.isEmpty ? 0 : cells.keys.fold<int>(0, (max, addr) => max < addr.row ? addr.row : max);
  
  int get columnCount => cells.isEmpty ? 0 : cells.keys.fold<int>(0, (max, addr) => max < addr.col ? addr.col : max);
  
  CellData? getCell(CellAddress address) => cells[address];
}
